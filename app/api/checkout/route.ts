...
// Flow:
//   1. Validate input
//   2. Create lead in KV/Supabase → ra orderId
...
export async function POST(req: Request) {
  ...
  const accountNumber = process.env.SEPAY_BANK_ACCOUNT_NUMBER || '1234567890';
  ...
  return NextResponse.json({
    ...
    bankInfo: { bank, accountNumber, accountName: process.env.SEPAY_ACCOUNT_NAME || 'NGUYEN VAN A' },
    content: orderId,
    qrUrl,
  });
}
